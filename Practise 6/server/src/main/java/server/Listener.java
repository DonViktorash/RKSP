    ...
    public void start() throws IOException, InterruptedException {
        ...
        try (ServerSocket server = new ServerSocket(port)) {
            ...
            while (!client.isClosed()) {
                ...
                // сервер ждёт в канале чтения (input stream) получения данных клиента
                String entry = in.readUTF();

                // после получения данных считывает их
                System.out.println("Клиент прислал: " + entry);

                // если клиент прислал кодовое слово quit - сервер выходит из цикла и закрывает все сокеты и соединения
